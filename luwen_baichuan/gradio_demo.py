...
tokenizer = AutoTokenizer.from_pretrained('output/zju_model_v125k/', trust_remote_code=True)
eos_token_ids = [tokenizer.eos_token_id]

base_model = AutoModelForCausalLM.from_pretrained(
    # args.base_model, 
    'output/zju_model_v125k/',
...
with gr.Blocks() as demo:
    gr.HTML("""<h1 align="center">Chinese BAICHUAN & Alpaca LLM</h1>""")
    current_file_path = os.path.abspath(os.path.dirname(__file__))
    # gr.Image(f'pics/banner.png', label = 'Chinese BAICHUAN & Alpaca LLM')
    # gr.Markdown("> 为了促进大模型在中文NLP社区的开放研究，本项目开源了中文BAICHUAN模型和指令精调的Alpaca大模型。这些模型在原版BAICHUAN的基础上扩充了中文词表并使用了中文数据进行二次预训练，进一步提升了中文基础语义理解能力。同时，中文Alpaca模型进一步使用了中文指令数据进行精调，显著提升了模型对指令的理解和执行能力")
    chatbot = gr.Chatbot()
    with gr.Row():
...
demo.queue().launch(share=False, inbrowser=True, server_name = '0.0.0.0', server_port=12011)
